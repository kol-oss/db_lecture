...
**REST (REpresentational State Transfer)** - це архітектурний стиль, що описує взаємодію фронт- та бекенд частин системи. Він визначає структуру та правила обробки запитів, встановлює формат пропускних точок (endpoints) та дозволяє уніфікувати звертання до серверної частини застосунку.

Кожен архітектурний стиль має особливості та найпопулярніші практики, що відокремлюють його від інших. Застосунки, які задовольняють принципами у архітектурі REST, називають RESTful-сервісами.
...
Наступні чотири обмеження можуть допомогти уніфікувати взаємодію у RESTful сервісах:

* Identification of resources (Визначення ресурсів) - Ресурсом в концепції REST API називається певна логічна сутність, для якої можна виокремити властивості та методи. Перелік таких ресурсів повинен бути чітко визначеним, а кожен ресурс повинен мати постійний ідентифікатор (URI).

* Manipulation of resources through representations (Взаємодія з ресурсами через представлення) - взаємодія з сутностями відбувається через представлення (їх вигляд у програмному коді), що відображає актуальний стан ресурсу.
   
* Self-descriptive messages (Повноописуючі повідомлення) - кожен запит повинен мати достатньо інформації, аби система могла зробити все, що він запитує.
   
* HATEOAS (Hypermedia as the engine of application state) (Гіпермедіа як рушій стану додатку) - вся інформація повинна передаватися через спеціалізовані частини запиту: заголовки, параметри строки запиту, тіло (body). Все це разом називається гіпермедіа.

### 2. Client-server (Клієнт-серверна взаємодія) 
...
### 1.3. Stateless (Без збереження стану)
...
Якщо відповідь можна кешувати, клієнтська програма отримує право повторно використовувати дані відповіді пізніше для еквівалентних запитів і протягом визначеного періоду.

### 1.5. Layred system (Багатошарова система)
...
Станом на сьогодні, саме JSON – стандарт для передачі даних, і його може використовувати майже кожна мережева технологія. Тож відповіді серверу мають формуватися з використанням цього формату.
...
Не слід використовувати дієслова у шляхах до ключових точок (endpoint’ів). Замість них кращим варіантом є іменники, які представляють сутність, яку ми отримуємо або з якою маніпулюємо.
...
Дія повинна бути вказана методом HTTP-запиту, який ми робимо. Найпоширеніші методи включають:
...
Наприклад, створити ендпойнти можна таким чином:
...
При проєктуванні кінцевих точок має сенс групувати ті з них, які містять пов'язану інформацію. Тобто, якщо один об'єкт може містити інший об'єкт, слід створити кінцеву точку, щоб відобразити це.

Це хороша практика, незалежно від того, чи дані у вашій БД структуровані таким чином. Також доцільно уникати дзеркального відображення структури бази даних у кінцевих точках, щоб не надавати зловмисникам додаткову інформацію.
...
Якщо кожна стаття має власні коментарі, то така структура має сенс, оскільки коментарі є дочірніми об'єктами статей. В іншому випадку це лише заплутає користувача.

### 4. Обробка помилок
Щоб усунути плутанину при виникненні помилки, потрібно обробляти її коректно і повертати коди HTTP-відповідей, які вказують, яка саме помилка сталася. Це дозволить користувачам, або розробникам API зрозуміти проблему, яка виникла.

Поширені коди статусу HTTP помилок включають:

* 400 Bad Request - це означає, що вхідні дані на стороні клієнта не пройшли валідацію;
* 401 Unauthorized - означає, що користувач не має дозволу на доступ до ресурсу;
* 403 Forbidden - означає, що користувач автентифікований, але йому заборонено доступ до ресурсу;
* 404 Not Found - означає, що ресурс не знайдено;
* 500 Internal Server Error - Це типова помилка сервера;
* 502 Bad Gateway - вказує на недійсну відповідь від сервера;
* 503 Service Unavailable - вказує на те, що на стороні сервера сталося щось непередбачуване.
...
### 5. Фільтрування, сортування та пагінації
...
У наведеному вище коді ми використовуємо змінну req.query для отримання параметрів запиту. Потім ми витягуємо значення властивостей, деструктуруючи окремі параметри запиту у змінні за допомогою синтаксису деструктуризації. Далі запускаємо фільтр для кожного значення параметра запиту, щоб знайти елементи, які ми хочемо повернути.
...
Ми також можемо вказати поля для сортування в рядку запиту.
...
Потрібно мати різні версії API, якщо збираєтеся вносити в них зміни, які можуть порушити роботу клієнтів. Версіонування можна зробити за семантичною версією major.minor.patch, як це робиться зараз у більшості додатків.
